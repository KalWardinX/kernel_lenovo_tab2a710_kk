...
#define CFG_LEGACY_NAND   (1) // for interface config
...
#define EFUSE_RANDOM_CFG	((volatile u32 *)(0xF02061c0))
...
u32 mtk_nand_page_transform(struct mtd_info *mtd, struct nand_chip *chip, u32 page, u32* blk, u32* map_blk)
{
	...
	//MSG(INIT , "[CCCCCHHHHHIIIIEEENNNWWWEEEIII]%d, %x\n",page,logical_address);
	if(MLC_DEVICE)
	{
...
//set feature
		...
		feature_set->Interface.address, &feature_set->Interface.feature,\
...
//setup register
		...
		mb();
		clkmux_sel(MT_MUX_NFI2X,clksrc,"NFI");
		mb();
		//DRV_WriteReg32(GPIO_BASE+0xC30, DRV_Reg32(GPIO_BASE+0xC30) & 0xbbfb);
		//DRV_WriteReg32(GPIO_BASE+0xC40, DRV_Reg32(GPIO_BASE+0xC40) & 0xbbbb);
		//DRV_WriteReg32(GPIO_BASE+0xC50, DRV_Reg32(GPIO_BASE+0xC50) & 0xfffb);
...
#if CFG_RANDOMIZER 
static int mtk_nand_randomizer_config(struct gRandConfig *conf, u32 page, u32 secindex, int fgPage)
{
    u32 u4NFI_CFG = 0;
	u32 u4NFI_RAN_CFG = 0;
	u4NFI_CFG = DRV_Reg32(NFI_CNFG_REG16);
    u4NFI_RAN_CFG = DRV_Reg32(NFI_RANDOM_CNFG_REG32);
	//if(use_randomizer)
	{
	    DRV_WriteReg32(NFI_ENMPTY_THRESH_REG32, 40); // empty threshold 40
		if(conf->type == RAND_TYPE_SAMSUNG) //samsung randomizer
		{
			#ifdef SS_RANDOM_EN
			u32 i = 0;
			u32 u4Pages = g_block_size / g_page_size;
			u4NFI_RAN_CFG &= ~SEED_MASK << EN_SEED_SHIFT;
		    u4NFI_RAN_CFG &= ~SEED_MASK << DE_SEED_SHIFT;
			for(i = 0; i < 6; i++)
			{
			    //clear TSB seed  to avoid S/TSB change side effect
                DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32 + i, 0);
                DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32 + i, 0);
			}		
			u4NFI_CFG |= CNFG_RAN_SEL;
			if(u4Pages <= SS_SEED_NUM)
			{
				u4NFI_RAN_CFG |= RAN_CNFG_ENCODE_SEED(SS_RANDOM_SEED[page & (u4Pages-1)]) | RAN_CNFG_ENCODE_EN;
				u4NFI_RAN_CFG |= RAN_CNFG_DECODE_SEED(SS_RANDOM_SEED[page & (u4Pages-1)]) | RAN_CNFG_DECODE_EN;
			}
			else
			{
			...
			#else
			MSG(INIT, "[NAND]Not config samsung randomizer!\n");
			#endif
		}
		else if(conf->type == RAND_TYPE_TOSHIBA) //toshiba randomizer
		{
			#ifdef TSB_RANDOM_EN
			DRV_WriteReg32(NFI_RANDOM_CNFG_REG32, 0); //clear SS seed to avoid S/TSB change side effect
			...
			{
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32, (TSB_RANDOM_SEED0[page % 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[page % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+1, TSB_RANDOM_SEED2[page % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+2, TSB_RANDOM_SEED3[page % 16] & 0x7FFFFF);
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+3, (TSB_RANDOM_SEED0[(page + 1)% 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[(page + 1) % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+4, TSB_RANDOM_SEED2[(page + 1) % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+5, TSB_RANDOM_SEED3[(page + 1) % 16] & 0x7FFFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32, (TSB_RANDOM_SEED0[page % 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[page % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+1, TSB_RANDOM_SEED2[page % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+2, TSB_RANDOM_SEED3[page % 16] & 0x7FFFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+3, (TSB_RANDOM_SEED0[(page + 1)% 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[(page + 1) % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+4, TSB_RANDOM_SEED2[(page + 1) % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+5, TSB_RANDOM_SEED3[(page + 1) % 16] & 0x7FFFFF);
			}
			else //reload seed for each sector
			{
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32, (TSB_RANDOM_SEED0[(page + secindex) % 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[(page + secindex) % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+1, TSB_RANDOM_SEED2[(page + secindex) % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+2, TSB_RANDOM_SEED3[(page + secindex) % 16] & 0x7FFFFF);
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+3, (TSB_RANDOM_SEED0[((page + secindex) + 1)% 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[((page + secindex) + 1) % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+4, TSB_RANDOM_SEED2[((page + secindex) + 1) % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_ENSEED01_TS_REG32+5, TSB_RANDOM_SEED3[((page + secindex) + 1) % 16] & 0x7FFFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32, (TSB_RANDOM_SEED0[(page + secindex) % 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[(page + secindex) % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+1, TSB_RANDOM_SEED2[(page + secindex) % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+2, TSB_RANDOM_SEED3[(page + secindex) % 16] & 0x7FFFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+3, (TSB_RANDOM_SEED0[((page + secindex) + 1)% 16] & 0x1FFF) | ((TSB_RANDOM_SEED1[((page + secindex) + 1) % 16] & 0x1FFFF) << 15));
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+4, TSB_RANDOM_SEED2[((page + secindex) + 1) % 16] & 0x7FFFF);
				DRV_WriteReg32(NFI_RANDOM_DESEED01_TS_REG32+5, TSB_RANDOM_SEED3[((page + secindex) + 1) % 16] & 0x7FFFFF);
			}
			#else
			MSG(INIT, "[NAND]Not config toshiba randomizer!\n");
			#endif	
		}
		else
		{
            MSG(INIT, "[NAND]Not found randomizer TYPE!\n");
		}
...
static void mtk_nand_turn_on_randomizer(u32 page, u32 secindex, int fgPage)
{
	//struct gRandConfig *conf = &devinfo.feature_set.randConfig;
	//kal_uint32   nfi_ran_cnfg = 0;
	mtk_nand_randomizer_config(&devinfo.feature_set.randConfig, page, secindex, fgPage);
	//nfi_ran_cnfg = DRV_Reg32(NFI_RANDOM_CNFG_REG32);
...
static void mtk_nand_turn_off_randomizer(void)
{
	kal_uint32   nfi_ran_cnfg = 0;
	nfi_ran_cnfg = DRV_Reg32(NFI_RANDOM_CNFG_REG32);
	nfi_ran_cnfg &= ~(RAN_CNFG_ENCODE_EN | RAN_CNFG_DECODE_EN);
	DRV_WriteReg32(NFI_RANDOM_CNFG_REG32, nfi_ran_cnfg);
}
#else
#define mtk_nand_randomizer_config(conf, page, secindex, fgPage)
#define mtk_nand_israndomizeron() (FALSE)
#define mtk_nand_turn_on_randomizer(page, secindex, fgPage)
...
int mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
	...
	do{
		if(use_randomizer && u4RowAddr >=  RAND_START_ADDR)
			mtk_nand_turn_on_randomizer(u4RowAddr, u4SecNum, FALSE);
...
static uint8_t mtk_nand_read_byte(struct mtd_info *mtd)
{
	...
	{
	 	g_brandstatus = FALSE;
	  	mtk_nand_turn_on_randomizer(0, 0, FALSE);
	}*/
...
#define EFUSE_GPIO_CFG	((volatile u32 *)(0xF02061c0))
...
static void mtk_nand_gpio_init(void)
{
    ...
    DRV_WriteReg32(GPIO_BASE+0xe20, DRV_Reg32(GPIO_BASE+0xe20) | 0x5 | (0x5 << 12));//NFI_BIAS_CTRL, temp solution
}
...
static int mtk_nand_probe(struct platform_device *pdev)
{
    ...
    for (j = 0x400; j< 0x7A0; j++)
    {
        ...
        for (k = 0; k < p; k++)
        {
            if(mtk_nand_write_page(mtd, nand_chip,(u8 *)patternbuff, j*p+k, 0, 0))
		...
		mtk_nand_SetFeature(mtd, (u16) feature_set->sfeatureCmd, \
		feature_set->Async_timing.address, &val,\
		sizeof(feature_set->Async_timing.feature));
		mtk_nand_GetFeature(mtd, feature_set->gfeatureCmd, \
		feature_set->Async_timing.address, &val,4);
        printk("[ASYNC Interface]0x%X\n", val);
        err = mtk_nand_interface_config(mtd);
		MSG(INIT, "[nand_interface_config] %d\n",err);
    }    
